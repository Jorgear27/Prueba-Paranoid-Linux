...
/**
 * @brief Construye un grafo Market conectado con pesos que fuerzan muchas pasadas de Bellman-Ford
 *
...
json buildSyntheticMarketGraph(int numNodes, unsigned seed = 42)
{
    ...
    for (int i = 0; i < numNodes; ++i)
    {
        ...
        node["node_location"] = {{"latitude", static_cast<double>(i)}, {"longitude", static_cast<double>(i)}};
        node["is_secure"] = true;
        node["is_active"] = true;
...
static void printHeader()
{
    ...
    printSeparator();
#ifdef _OPENMP
    std::cout << "  OpenMP disponible. Max threads del sistema: " << omp_get_max_threads() << "\n";
#else
...
RunStats runParallel(BellmanFord& bf, const Graph& graph, const std::string& source, int runs, int threads)
{
    ...
    BellmanFord::ProfilingStats ps;
    for (int r = 0; r < runs; ++r)
        bf.computeParallel(graph, source, &ps, threads), times.push_back(ps.total_time_ms);
...
int main(int argc, char* argv[])
{
    ...
    printHeader();
    std::cout << "  Nodos: " << numNodes << "  |  " << "Aristas: " << numNodes * (numNodes - 1) << "  |  "
    ...
    std::cout << "  Min / Max      : " << serialStats.min_ms << " / " << serialStats.max_ms << " ms\n";
    std::cout << "  Desviación std : " << serialStats.stddev << " ms\n";
    std::cout << "  Pasadas (last) : " << serialStats.last.passes_executed << "\n";
    std::cout << "  Tiempo serial (init+neg-cycle): " << serialStats.last.serial_time_ms << " ms\n";
    std::cout << "  Tiempo paralelo (inner loop)  : " << serialStats.last.parallel_time_ms << " ms\n";

    const double fracSerial = (serialStats.mean_ms > 0) ? serialStats.last.serial_time_ms / serialStats.mean_ms : 0.0;
    const double fracParallel = 1.0 - fracSerial;

    std::cout << "\n  ── Fracciones de tiempo (Ley de Amdahl) ──\n";
    std::cout << "  Fracción serial   S = " << std::setprecision(4) << fracSerial * 100.0 << "%\n";
    ...
    std::cout << "\n[2/3] Midiendo PARALELO con " << threadCounts.size() << " configuraciones de threads...\n\n";

    std::cout << std::setw(10) << "Threads" << std::setw(14) << "Tiempo(ms)" << std::setw(12) << "Speedup"
              << std::setw(14) << "Eficiencia" << std::setw(20) << "Speedup Amdahl" << "\n";
    printSeparator(70);
    ...
    for (int t : threadCounts)
    {
        ...
        std::cout << std::setw(10) << t << std::setw(14) << std::setprecision(3) << ps.mean_ms << std::setw(12)
                  << std::setprecision(3) << speedup << std::setw(13) << std::setprecision(3) << efficiency * 100.0
    ...
    std::cout << "  Fracción serial medida : " << std::setprecision(4) << fracSerial * 100.0 << "%\n";
    const double maxSpeedup = (fracSerial > 0) ? 1.0 / fracSerial : 999.9;
    std::cout << "  Speedup máximo teórico : " << std::setprecision(2) << maxSpeedup << "x  (con infinitos threads)\n";
    ...
    std::cout << std::setw(8) << "Nodos" << std::setw(12) << "Aristas" << std::setw(14) << "Serial(ms)" << std::setw(16)
              << "Paralelo(ms)" << std::setw(12) << "Speedup" << std::setw(10) << "Pasadas" << "\n";
    printSeparator(72);
    ...
    for (int n : {50, 100, 200, 400, 600, 800})
    {
        ...
        for (int r = 0; r < R2; ++r)
        {
            bf.computeSerial(g2, "M0", &s2);
            ts += s2.total_time_ms;
            bf.computeParallel(g2, "M0", &p2);
            tp += p2.total_time_ms;
        }
        ts /= R2;
        tp /= R2;
        ...
        std::cout << std::setw(8) << n << std::setw(12) << (n * (n - 1)) << std::setw(14) << std::setprecision(2) << ts
                  << std::setw(16) << std::setprecision(2) << tp << std::setw(12) << std::setprecision(2) << sp
