 ...
 */
std::string handleProfilingBf(const Graph& graph,
                               ...
                               int& statusCode)
{
    ...
    for (int r = 0; r < RUNS; ++r)
    {
        ...
        if (r == RUNS - 1) { serialStats = s; parallelStats = p; }
    }
    ...
    const double fracSerial = (meanSerial > 0)
        ? serialStats.serial_time_ms / meanSerial : 0.0;
